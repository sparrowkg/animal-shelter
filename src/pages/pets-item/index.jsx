...
function getRandomVolunteer(volunteers) {
  const randomIndex = Math.floor(Math.random() * volunteers.length);
  return volunteers[randomIndex];
}
...
function PetsItemPage() {
  ...
  useEffect(() => {
    ...
    fetch('http://35.195.136.135/api/volunteer/')
      .then(res => res.json())
      .then((data) => {
        setVolunteers(data)
      })
  ...
  const randomVolunteer = getRandomVolunteer(volunteers);
  ...
  return (
    <div className='container'>
      <div className='petsitempage_block'>
        ...
        <div>
          <button className='button-pet-item' onClick={() => history(-1)}>
            <svg className='close_svg' width="28" height="28" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <div className='petsitempage_contac'>
            <h4 className='petsitempage_volonter'>ВОЛОНТЕР: {randomVolunteer?.name}</h4>
            <h4 className='petsitempage_contacts'>ТЕЛЕФОН: {randomVolunteer?.phone_number}</h4>
          </div>
        </div>
      </div>
